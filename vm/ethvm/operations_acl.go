...
func gasSLoadEIP2929(vm *ETHVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	loc := stack.peek()
...
func gasExtCodeCopyEIP2929(vm *ETHVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
...
// - extcodesize,
// - (ext) balance
func gasEip2929AccountCheck(vm *ETHVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	addr := metatypes.Address(stack.peek().Bytes20())
	// Check slot presence in the access list
	if !vm.StateDB.AddressInAccessList(addr) {
		// If the caller cannot afford the cost, this change will be rolled back
...
func makeSelfdestructGasFn(refundsEnabled bool) gasFunc {
	gasFunc := func(vm *ETHVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
		var (
			gas     uint64
		...
		if vm.StateDB.Empty(account) && vm.StateDB.GetBalance(contract.Account()).Sign() != 0 {
